cy = as.numeric(commandArgs(trailingOnly=TRUE))
...
js <- nimbleCode({
    ...
    for (j in 1:Nw) {
        wt.hat[j] <- birthWt + beta.wt * (julianDay[j] - bDate[nimbleID[j]])
    ...
    # prior wt-BD model
    birthWt~dnorm(10,sd=2)
    beta.wt ~ dnorm(0.5, sd=0.2)
    sd.mass ~ dunif(0, 5)
    sd.bd ~ dunif(1, 15)     # at least a 4 day range in BD , max 60
    mu.bd~ dnorm(140, sd=10)
    ...
    for (i in 1:M){ # first
        ...
        z[i,1] ~ dbern(nu[1]) #* born # conditional entry prob
        # Observation process
...
inits <- function(){
    l=list(psi = runif(1, 0, 1), w=w.init,z = z.init, psi=runif(1,0.3,0.6),
           nu=runif(55,.1,.2),
           ...
           bDate=sample(135:148,size =  df.syjs[[cy]]$const$M2,replace = T))
...
parameters <-c(#"p.mean",
               ...
               'phi.betaWeaned',
               'phi.betaYear',
               'N',"Ns1",'Nsuper','B','NsuperS1',"psi"#,'nu','b'
...
newOut <- nimbleMCMC(
    ...
    niter = 20000, thin = 10,nburnin = 10000, # 1000 iterations left to estimate parameters
    WAIC=TRUE,
    summary = TRUE,
...
saveRDS(newOut,file=paste0('cache/20230516-16h_js1c_',cy,'.rds'),compress = 'xz')
