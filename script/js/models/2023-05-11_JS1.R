...
df.js <- read_rds("cache/df_js.rds")
# nimble model
js <- nimbleCode({
    ...
    for(year in 1:16){
        Nsuper_site0[year] <- sum(Nalive[1:M]*(1-site_int[1:M])*(year_int[1:M]==year)) # Superpopulation size
        Nsuper_site1[year] <- sum(Nalive[1:M]*site_int[1:M]*(year_int[1:M]==year)) # Superpopulation size
    }
...
inits <- function() {
    ...
    l$bDate=ifelse(l$bDate>df.js$const$first.bd,
                   df.js$const$first.bd-2,
                   l$bDate)
    return(l)
}

# parameters monitored
parameters <-c("p.mean","phi.mean","p.betaWeaned","beta.wt","bDate","mu.bd","sd.bd","sd.mass",
                     "weanSurv",'dailySurv',
               'p.betaSite','p.betaYear',"sd.p",'phi.sex','phi.betaWeaned','phi.betaYear','phi.occ',
...
saveRDS(newOut,file=paste0('cache/20230512_js1.rds'),compress = 'xz')
