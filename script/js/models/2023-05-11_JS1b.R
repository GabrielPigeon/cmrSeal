...
js <- nimbleCode({
    ...
    for (t in 1:n.occasions){
        ...
        for(year in 1:16){
        N_YerSite[year,1,t] <- sum(u[yrRAnge[year,1]:yrRAnge[year,2],t]*(1-site_int[yrRAnge[year,1]:yrRAnge[year,2]]))
        N_YerSite[year,2,t] <- sum(u[yrRAnge[year,1]:yrRAnge[year,2],t]*site_int[yrRAnge[year,1]:yrRAnge[year,2]])
        }
        B[t] <- sum(recruit[yrRAnge[year,1]:yrRAnge[year,2],t]) # Number of entries
    } #t
    for (i in 1:M){
    ...
    for(year in 1:16){
        Nsuper_site0[year] <- sum(Nalive[yrRAnge[year,1]:yrRAnge[year,2]]*(1-site_int[yrRAnge[year,1]:yrRAnge[year,2]])) # Superpopulation size
        Nsuper_site1[year] <- sum(Nalive[yrRAnge[year,1]:yrRAnge[year,2]]*site_int[yrRAnge[year,1]:yrRAnge[year,2]]) # Superpopulation size
    }
