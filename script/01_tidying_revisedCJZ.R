...
# revisions - create capture history ch - do not split by year -----------------------------------------

# make two lists, one per site
pup.data35$myID <- droplevels(pup.data35$myID)
pupData_list_2sites<- pup.data35 %>% split(., .$mySite)

# assign correct class 
site=levels(as.factor(unique(pup.data35$mySite)))
tmpSite=length(site)

# create 0 
reps=rep(0,tmpSite)

# create the matrix of 0 and 1 per ID and unique value of Julian day, per site
# pup_ch_l <- list()
# for(j in 1:length(site)){
#     x=pup.data35[which(pup.data35$mySite==site[j]),]
#     y=as.character(site[j])
#     x$myID<-droplevels(x$myID) # not sure why - doesn't drop levels per year in list
#     pup_ch_l[[j]]=with(x,table(myID,factor(julianDay))) # do not take date - that takes the year
#     colnames(pup_ch_l[[j]]) <- NULL# this is what transforms into a matrix of 0-1
#     names(pup_ch_l)[[j]]=paste(y)
#     print(dim(pup_ch_l[[j]]))
#     pupData_list_2sites[[j]]$nimbleID <- match(pupData_list_2sites[[j]]$myID, rownames(pup_ch_l[[j]]), nomatch = NA_integer_, incomparables = NULL)
# }
...
for (j in 1:length(site)) {
    ...
    all_julian_days <- factor(min(x$julianDay):max(x$julianDay))
    pup_ch_l[[j]] <- with(x, table(myID, factor(julianDay, levels = all_julian_days)))
    colnames(pup_ch_l[[j]]) <- NULL
    names(pup_ch_l)[[j]] <- y
    print(dim(pup_ch_l[[j]]))
    pupData_list_2sites[[j]]$nimbleID <- match(pupData_list_2sites[[j]]$myID, rownames(pup_ch_l[[j]]), nomatch = NA_integer_, incomparables = NULL)
}
...
# Bic is 976 x 51
# Metis is 397 by 50
View(pupData_list_2sites[["bic"]])
View(pupData_list_2sites[["metis"]])
# end verificatin



# save(pup_ch_l,pupData_list_2sites,pup.data35,file = "data/mine/2023-03-28_CjsDf_revised.RData")
