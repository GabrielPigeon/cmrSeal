...
cjs <- nimbleCode({
    ...
    for (i in 1:nind) {
        bDate[i] ~ T(dnorm(mu.bd[year_int[i]], sd = sd.bd), 120, first.bd[i]) 
  ...
  # prior wt-BD model
    ...
    sd.p~dunif(0, 1.5)
    for(y in 1:16) {
        mu.bd[year_int[i]]~ dnorm(130, 0.001)
...
saveRDS(chainOut,file=paste0('~/projects/def-pelleti2/renl2702/phoques/outputs/20230512_1012_fullCJS.rds'),compress = 'xz')
