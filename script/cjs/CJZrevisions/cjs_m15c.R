...
cjs <- nimbleCode({
    ...
    for (i in 1:nind) { # individuals
        ...
        for(t in 1:n.occasions){
            logit(p[i,t]) <- p.betaYear[site_int[i]+1,year_int[i]]+ 
  ...
  # prior wt-BD model
    ...
    for(i in 1:16) { # things that change by year
      p.betaYear[1,i]~dlogis(0, 1)
      p.betaYear[2,i]~dlogis(0, 1)
      
      phi.betaYear[1,i]~dnorm(phi.mean[1],sd=sd.phi.yr) #sd estimated so it's a Random Effect
      phi.betaYear[2,i]~dnorm(phi.mean[2],sd=sd.phi.yr)
      for(t in 1:n.occasions){ #things that change by occasion
        phi.occ[i,t]~dnorm(0, sd=sd.occ) 
        p.occ[i,t]~dnorm(0, sd=sd.Pocc)
      }
    }
...
parameters <-    c(
        "phi.mean",
        "sd.phi.yr","sd.Pocc","sd.occ","phi.betaWeaned",
        "phi.betaYear","phi.sex","phi.occ",
      #  "z",
        "p.betaWeaned",
        "beta.wt",
        "bDate",
...
chainOut <- nimbleMCMC(
    ...
    niter = 2*1000+1000, thin = 2,nburnin = 1000, # 1000 iterations left to estimate parameters
    WAIC=TRUE,
    summary = TRUE,
...
write_rds(chainOut,file = '20230507_m15c.rds',compress = 'xz')
